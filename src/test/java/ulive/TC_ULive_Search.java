...
/**
 * 
 * This test case tests the search functionality with autocomplete load ulive
 * site, on search textfield of menu bar, search Giada verify autocomplete
 * results compared to autocomplete api calls click submit. verify Giada text on
 * Search page. Verify first fridge magnet is displayed on Search page.
 * 
 * @author slu
 * 
 */
public class TC_ULive_Search extends UliveBaseClass {

	private String searchTerm = "giada";

	@Parameters({ "browser" })
	public TC_ULive_Search(String browser) {
		super(browser);
	}
	...
	public void test() throws Exception {
		...
		while (count.hasNext()) {
			...
			if (existsInArray(keys, searchResult.getText())) {
				...
				System.out.println(searchResult.getText() + " does not match api call");
				PeonyAssertion.verifyTrue(false, searchResult.getText()
